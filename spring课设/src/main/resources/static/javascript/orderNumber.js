...
function order(doctorAccount, date, week, time) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data, function (i,number) {//解析json数据
                ...
                html += "<a onclick=\"areYouSure(" + doctorAccount + ",'" + date + "'," + time + "," + week + "," + number + ")\" >" +
...
function areYouSure(doctorAccount, date, time, week, number) {
    var t;
    if(time == 1)
        t = "上午";
    else
    ...
    layer.confirm("预约号码为：" + doctorAccount + " " + date + " " + t  + " " + number + "号", {
        ...
        $(location).attr('href','/doOrder?doctorAccount=' + doctorAccount + '&date=' + date + '&time=' + time + '&week=' + week + '&number=' + number);
    }, function(){
        window.pener = null;
        window.close();
    });
